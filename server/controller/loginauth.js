            ...
            .then((passwordMatch)=>{
                if(passwordMatch){
                   const token =  jwt.sign({email:data[0].email},process.env.SECRET_KEY);
                        res.json({token,message:"Successfully Logged In with phone as ID",name:data[0].name})
                }else{
        ...
        .then((data)=>{
            if(data.length !== 0){
                ...
                .then((passwordMatch)=>{
                    if(passwordMatch){
                      const token =  jwt.sign({email:data[0].email},process.env.SECRET_KEY);
                            res.json({token,message:"Successfully Logged In with email as ID",name:data[0].name});
                    }else{
                        return res.json({message:'Incorrect password !'})
                    }
                })
